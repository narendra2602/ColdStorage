...
public class PrtGroupOpt extends BaseClass implements ActionListener 
{
	...
	YearDto yd;
	private Vector partyNames,productNames,groupNames;
	TransportDAO pdao ;
	Font font,fontHindi;
	...
	private JLabel lblPartyCombo;
	private JPanel panel_3;

	public  PrtGroupOpt(String nm,String repNm)
	{
				...
				if (key == KeyEvent.VK_ENTER) {
					...
					case 1: 
						...
						if(isValidDate(date))
						{
							...
							{
								JOptionPane.showMessageDialog(PrtGroupOpt.this,"Date range should be in between: "
		...
		selectiveProduct.setBounds(42, 250, 182, 23);
		getContentPane().add(selectiveProduct);
		
		allProducts = new JRadioButton("All Products");
		allProducts.setBounds(220, 250, 132, 23);
		...
		String [] invColName=	{"", "Product Name",""};
		String datax[][]={{}};
...
//		smonth.addActionListener(this);
		...
		partyCombo = new JComboBox(partyNames);
		partyCombo.setBounds(103, 209, 269, 22);
		getContentPane().add(partyCombo);

		lblPartyCombo = new JLabel("Select:");
		lblPartyCombo.setBounds(43, 210, 95, 20);
		getContentPane().add(lblPartyCombo);
...
//		panel_1.setBorder(new LineBorder(new Color(0, 0, 0)));
	...
	public void fillItemDataPrd(boolean res)
	{
		...
		int groupCode=0;
		Vector prtVec =null;
		prtVec = (Vector) groupNames;
		int size= prtVec.size();
		Vector dd = null;
		for(int i =0;i<size;i++)
		{
			prt = (GroupDto) prtVec.get(i);
				dd = new Vector();
				dd.add(new Boolean(res));
				dd.add(prt.getGp_name());
				dd.add(prt.getGp_code());
	 ...
	 public void callPrint(String btnnm )
	 {
		 try
		 {
			 ...
			 {
				 JOptionPane.showMessageDialog(this, "Please Select Groups!!!!");
			 }
			 else
			 {
//				 MonthDto sdt = (MonthDto) smonth.getSelectedItem(); 
				 ...
				 int doc_type=60;
				  ...
				  int code = partyCombo.isEnabled()?((TransportDto)partyCombo.getSelectedItem()).getAccount_no():0;
				 
				// div_code mai 99 pass kiya for running group option
				 ...
				 Object ob = (Object)constructor.newInstance(yd.getYearcode(),loginDt.getDepo_code(),99,sdate.getText(),edate.getText(),btnnm,loginDt.getDrvnm(),loginDt.getPrinternm(),loginDt.getBrnnm(),code,noOfSelectedRow,doc_type);
				 resetAll();
				 dispose();
			 }
