...
public class WrittenOffEntry extends BaseClass implements ActionListener {
...
//		String[] crDrColName = { "Marka No", "Qty", "Manzil", "Gala No"};
		String[] crDrColName = { "Slip No","Date","Item","Pack","Balance Qty", "Balance Weight","Out Qty", "WrittenOff Qty", "WrittenOff Weight","Manzil","Gala", "","","","","","","","",""};
		String cdDrData[][] = {{}};
		CrTableModel = new DefaultTableModel(cdDrData, crDrColName) {
			...
			public boolean isCellEditable(int row, int column) 
			{
				...
				boolean ans=false;
				if(option==1 && (column==6 ||column==7 || column==8 || column==9 || column==10))
				{
					return true;
				}
				else
...
*/
		lblLastNo = new JLabel("Last WrittenOff No:");
...
//		lblDispatchEntry.setBounds(331, 94, 382, 22);
				...
				if(evt.getKeyCode() == KeyEvent.VK_ENTER)
				{
					...
					} else 
					{
						...
						if (prtyDto == null)
						{
							...
							rcp =  idao.getInvDetailPartyWritten(party,"",loginDt.getFin_year(),2);
						}
					if(rcp==null)
					{
						clearAll();
						JOptionPane.showMessageDialog(WrittenOffEntry.this,"No Record for Khata No. "+party,"Record not found",JOptionPane.INFORMATION_MESSAGE);						
			...
			public void keyPressed(KeyEvent evt) {
				...
				if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
					if(name.getText().length()==0)
						alertMessage(WrittenOffEntry.this, "Please select Party!");
					else
		...
		chl_no.addKeyListener(new KeyAdapter() 
		{
			...
			public void keyPressed(KeyEvent evt) 
			{
				...
				if(evt.getKeyCode() == KeyEvent.VK_ENTER)
				{
					...
					if (vouno==0)
					{
						JOptionPane.showMessageDialog(WrittenOffEntry.this,"Challan No. "+vouno,"Empty!!!!",JOptionPane.INFORMATION_MESSAGE);						
						chl_no.setText("");
						chl_no.requestFocus();
					}
					else
					{						
						rcp =  idao.getChallanDetail(chl_no.getText(),loginDt.getFin_year(),66,loginDt.getDepo_code(),loginDt.getDiv_code());
						...
						if(rcp!=null)
						{
							if(option==1)
							{
							JOptionPane.showMessageDialog(WrittenOffEntry.this,"Challan No. "+vouno,"Already Exists!!!!!",JOptionPane.INFORMATION_MESSAGE);						
							chl_no.setText("");
							chl_no.requestFocus();
							}
							else
						...
						{
							JOptionPane.showMessageDialog(WrittenOffEntry.this,"Challan No."+billNo," Not Found!!!!!",JOptionPane.INFORMATION_MESSAGE);						
							chl_no.setText("");
							chl_no.requestFocus();
						}
						evt.consume();
		...
		chl_date.addFocusListener(new FocusListener() {
			...
			public void focusLost(FocusEvent arg0) {
				...
				checkDateValidity(date, chl_date, CrDrTable, WrittenOffEntry.this);
				...
				if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
					if (column == 6)  // block qty
					{
						...
						else
						{
							...
							rqty = setIntNumber(CrDrTable.getValueAt(row, 6).toString());
							if(rqty>sqty && option==1)
							{
								alertMessage(WrittenOffEntry.this, "Quantity should not be greater than "+sqty);
								CrDrTable.setValueAt("", row, 6);
					...
					if (column == 7)  // qty
					{
						...
						else
						{
							...
							else
							{
								...
								CrDrTable.setValueAt(weight, row, 8);
								CrDrTable.changeSelection(row, 8, false, false);
								CrDrTable.editCellAt(row, 8);
					...
					if (column == 8)
					{
						if(option==0)
						{
						...
						else
							{
								...
								{
									alertMessage(WrittenOffEntry.this, "Weight should not be greater than "+sweight);
...
//		panel_1.setBounds(40, 186, 975, 518);
		...
		btnSave.addKeyListener(new KeyAdapter() {
			public void keyPressed(KeyEvent evt) {
				if (evt.getKeyCode() == KeyEvent.VK_ENTER) 
				{
					...
					if(ino!=0)
					{
						JOptionPane.showMessageDialog(WrittenOffEntry.this,"WrittenOff No "+billNo,"WrittenOff No",JOptionPane.INFORMATION_MESSAGE);
					}
					vac_code.requestFocus();
		...
		try {
			for (int i = 0; i < size; i++) {
				...
				if(qty>0)
				{
					...
					rcp1.setSqty(setIntNumber(col.get(4).toString()));
					rcp1.setWeight(setDoubleNumber(col.get(5).toString()));
					rcp1.setBlock_qty(setIntNumber(col.get(6).toString()));
					rcp1.setIssue_qty(setIntNumber(col.get(7).toString()));
					rcp1.setIssue_weight(setDoubleNumber(col.get(8).toString()));
					rcp1.setFloor_no(col.get(9).toString());
					rcp1.setBlock_no(col.get(10).toString());
					...
					rcp1.setMark_no((col.get(18).toString()));
					rcp1.setCategory_hindi(col.get(19).toString());
	...
	public int saveData() 
	{
		...
		if (option==3)
		{
	    		...
	    		int answer = JOptionPane.showOptionDialog(WrittenOffEntry.this, "Are You Sure : ","Confirm Yes/No",JOptionPane.YES_NO_OPTION,
...
//		vou_date.setText(sdf.format(rcp.getInv_date()));
	...
	public void itemTableUI() 
	{
		...
		CrDrTable.getColumnModel().getColumn(2).setPreferredWidth(30); // item
		CrDrTable.getColumnModel().getColumn(3).setPreferredWidth(30); // pack
		CrDrTable.getColumnModel().getColumn(4).setPreferredWidth(50); // balance qty
		CrDrTable.getColumnModel().getColumn(5).setPreferredWidth(60); // balance weight
		CrDrTable.getColumnModel().getColumn(6).setPreferredWidth(50); // block qty
		CrDrTable.getColumnModel().getColumn(7).setPreferredWidth(60); // wriiten qty
		CrDrTable.getColumnModel().getColumn(8).setPreferredWidth(80); // written weight
		CrDrTable.getColumnModel().getColumn(9).setPreferredWidth(40); // manzil
		CrDrTable.getColumnModel().getColumn(10).setPreferredWidth(40); // gala


		
		CrDrTable.getColumnModel().getColumn(11).setPreferredWidth(0);
		CrDrTable.getColumnModel().getColumn(11).setMinWidth(0); //spd_gp
		CrDrTable.getColumnModel().getColumn(11).setMaxWidth(0);

		
		CrDrTable.getColumnModel().getColumn(12).setPreferredWidth(0);
		CrDrTable.getColumnModel().getColumn(12).setMinWidth(0); // serial no
		CrDrTable.getColumnModel().getColumn(12).setMaxWidth(0);


		CrDrTable.getColumnModel().getColumn(13).setPreferredWidth(0);
		CrDrTable.getColumnModel().getColumn(13).setMinWidth(0); //  sprd_cd
		CrDrTable.getColumnModel().getColumn(13).setMaxWidth(0);


		CrDrTable.getColumnModel().getColumn(14).setPreferredWidth(0);
		CrDrTable.getColumnModel().getColumn(14).setMinWidth(0); // category_name in english
		...
		CrDrTable.getColumnModel().getColumn(18).setMinWidth(0); //mark no
		CrDrTable.getColumnModel().getColumn(18).setMaxWidth(0);

		
		CrDrTable.getColumnModel().getColumn(19).setPreferredWidth(0);
		CrDrTable.getColumnModel().getColumn(19).setMinWidth(0); // category in hindi
