...
public class InwardEntryOld2 extends BaseClass implements ActionListener {
	...
	private JLabel lblChqAmount, lblTaxable, lblCgst, lblSgst, lblIgst,
			lblGstTax;
	...
	private JLabel hintTextLabel, hintTextLabelcat, hintTextLabelgrp,
			hintTextLabelprd;
	...
	public InwardEntryOld2() {
		...
		/*
		 * //////////////invoce no table model///////////////////// String []
		 * invColName= {"Vou No.", "Name","",""}; String
		 * datax[][]={{},{},{},{},{
		 * },{},{},{},{},{},{},{},{},{},{},{},{},{},{},{}
		 * ,{},{},{},{},{},{},{},{},{}}; invNoTableModel= new
		 * DefaultTableModel(datax,invColName) { private static final long
		 * serialVersionUID = 1L; public boolean isCellEditable(int row, int
		 * column) { return false; } };
		 * 
		 ...
		 * //
		 * ////////////////////////////////////////////////////////////////////
		 * ///// invNoTable.getColumnModel().getColumn(0).setPreferredWidth(70);
		 * //contact inv no
		 * invNoTable.getColumnModel().getColumn(1).setPreferredWidth(120);
		 * //party name/////
		 * invNoTable.getColumnModel().getColumn(2).setMinWidth(0);
		 * //inv_no/////
		 * invNoTable.getColumnModel().getColumn(2).setMaxWidth(0);
		 * //inv_no/////
		 * invNoTable.getColumnModel().getColumn(3).setMinWidth(0); //table
		 * data///// invNoTable.getColumnModel().getColumn(3).setMaxWidth(0);
		 * //tabel data/////
		 */
		...
		categoryList.addKeyListener(new KeyAdapter() {
			public void keyPressed(KeyEvent evt) {
				if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
					...
					vouTable.setValueAt(catDto.getCategory_name_hindi(), rrow,
							0);
		...
		Packpane = new JScrollPane(packList,
				ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS,
				ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER);
		...
		product_name.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent evt) {
				...
				if (evt.getKeyCode() == KeyEvent.VK_ESCAPE) {
					...
					hintProductTextLabel.setVisible(false);
					searchFilter(product_name.getText(), productList,
							productNames);
					vouTable.editCellAt(rrow, 1);
					vouTable.requestFocus();
					evt.consume();
		...
		/*
		 * JScrollPane scrollPane = new JScrollPane(invNoTable,
		 * ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS,
		 * ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER);
...
/*		vou_no.addFocusListener(new FocusListener() {
...
*/
		...
		vou_date.addFocusListener(new FocusListener() {
			...
			public void focusLost(FocusEvent arg0) {
				...
				checkDateValidity(date, vou_date, vac_code, InwardEntryOld2.this);
		 ...
		 * branch.setForeground(Color.BLACK); branch.setFont(new Font("Tahoma",
		 * Font.BOLD, 22)); branch.setBounds(340, 66, 560, 24);
		...
		KeyListener keyListener = new KeyAdapter() {
			public void keyPressed(KeyEvent keyEvent) {
				...
				if (keyEvent.getKeyCode() == KeyEvent.VK_ENTER) {
					...
					case 1:
						/*
						 * String date=vou_date.getText(); if(isValidDate(date))
						 * { //lblinvalid.setVisible(false); // Check the Date
						 * Range for the Financial year if
						 * (isValidRange(date,loginDt
						 * .getFr_date(),loginDt.getTo_date())) {
						 * vac_code.requestFocus(); } else {
						 * JOptionPane.showMessageDialog
						 * (InwardEntry.this,"Date range should be in between: "
						 * + sdf.format(loginDt.getFr_date())+ " to " +
						 * sdf.format(loginDt.getTo_date()), "Date Range",
						 * JOptionPane.INFORMATION_MESSAGE);
						 * vou_date.setValue(null); checkDate(vou_date);
						 * vou_date.requestFocus();
						 * 
						 * }
						 * 
						 * 
						 * } else { lblinvalid.setText("invalid date");
						 * lblinvalid.setForeground(Color.red);
						 * lblinvalid.setVisible(true); vou_date.setValue(null);
						 * checkDate(vou_date); vou_date.requestFocus(); }
						 */
						vac_code.requestFocus();
						break;
					...
					case 8:
						break;
					case 9:
						weight.setText(formatter.format(setDoubleNumber(weight
								.getText())));
						rent_amt.setText(formatter
								.format(setDoubleNumber(weight.getText())
										* setDoubleNumber(rate.getText())));
					...
					case 10:
						...
						break;
					case 11:
						roundoff.setText(formatter
								.format(setDoubleNumber(roundoff.getText())));
						vamt = setDoubleNumber(rent_amt.getText())
								+ setDoubleNumber(roundoff.getText());
				...
				/*
				 * if (key == KeyEvent.VK_ESCAPE) { clearAll(); dispose();
				 * 
				 * }
				 */
		...
		// ////////////////////////////////////////////////////////////////////
		String[] crDrColName = { "Category", "Group", "Item", "Pack", "Rate",
				"Qty", "Weight", "Amount", "RST No", "", "", "" };
		String[][] cdDrData = { {}, };
		...
		// ////////////////////////////////////////////////////////////////////////
		vouTablePane = new JScrollPane(vouTable,
				ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS,
				ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER);
				...
				if (addRecord) {
					...
					if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
						...
						if (column == 1) {
							...
							try {
								qty = setIntNumber(vouTable.getValueAt(row, 5)
										.toString().trim());
							} catch (Exception e) {
								qty = 0;
							...
							try {
								qty = setIntNumber(vouTable.getValueAt(row, 6)
										.toString().trim());
							} catch (Exception e) {
								qty = 0;
							...
							if (qty == 0) {
								...
								vouTable.setValueAt(
										formatter.format(net * qty), row, 7);
								vouTable.changeSelection(row, 8, false, false);
								vouTable.editCellAt(row, 8);
							}
						}
							...
							try {
								qty = setIntNumber(vouTable.getValueAt(row, 8)
										.toString().trim());
							} catch (Exception e) {
								qty = 0;
							}
							if (qty == 0) {
								...
								vouTableModel.addRow(new Object[] {});
								vouTable.changeSelection(totRow, 0, false,
										false);
			...
			public void keyPressed(KeyEvent evt) {
				...
				if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
					if (name.getText().length() == 0)
						alertMessage(InwardEntryOld2.this, "Please select Party!");
					else
						vnart_1.requestFocus();
					evt.consume();
				}
...
//		vou_no.addKeyListener(keyListener);
		...
		vou_no.addKeyListener(new KeyAdapter() { public void
			keyPressed(KeyEvent evt) { if (evt.getKeyCode() ==
			KeyEvent.VK_ESCAPE) { clearAll(); dispose(); //System.exit(0);

			} if(evt.getKeyCode() == KeyEvent.VK_ENTER) { int vouno =
					setIntNumber(vou_no.getText().toString());

			if (vouno==0) {
				JOptionPane.showMessageDialog(InwardEntryOld2.this,"Slip No. "
						+vouno,"Empty!!!!",JOptionPane.INFORMATION_MESSAGE);
				vou_no.setText(""); vou_no.requestFocus(); } else { rcp =
						idao.getInvDetailNew(vouno,"",loginDt.getFin_year(),60,loginDt.getDiv_code());
				if(rcp==null) { option=1; clearAll(); }

				if(rcp!=null) {

					JOptionPane.showMessageDialog(InwardEntryOld2.this,"Slip No. "+vouno,
							"Already Exists!!!!!",JOptionPane.INFORMATION_MESSAGE);
					vou_no.setText(""); vou_no.requestFocus(); option=0; fillData(rcp);
					btnSave.setEnabled(true); vouTableModel.getDataVector().clear();
					vouTableModel.addRow(new Object[]
							{"","","","","","","","","","","",""}); vouTablePane.requestFocus();
							vouTable.requestFocus(); fillItemData(rcp);
							vouTable.changeSelection(0, 0, false, false); vouTable.editCellAt(0,
									0);


				} else vou_date.requestFocus();
				...
				if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
					if (addBool) {
						if (vac_code.getText().toString().trim().equals("")) {
							...
							hintTextLabel.setVisible(false);
							int getText = setIntNumber(vac_code.getText()
									.toString().trim());

							prtyDto = (TransportDto) partyMap.get(vac_code
							...
							if (prtyDto == null) {
								...
								vac_code.setText(String.valueOf(prtyDto
										.getAccount_no()));
								name.setText(prtyDto.getMac_name());
								vnart_1.setText(prtyDto.getMac_name_hindi());
						...
						alertMessage(InwardEntryOld2.this,
		...
		/*
		 * invNoTable.addKeyListener(new KeyAdapter() { public void
		 * keyPressed(KeyEvent evt) { int row = invNoTable.getSelectedRow();
		 * if(evt.getKeyCode() == KeyEvent.VK_ENTER) { rcp = (InvViewDto)
		 * invNoTable.getValueAt(row, 2); fillData(rcp);
		 * 
		 * evt.consume(); }
		 * 
		 * } });
		 * 
		 * 
		 * invNoTable.addMouseListener(new MouseListener() { public void
		 * mouseReleased(MouseEvent e){} public void mousePressed(MouseEvent e)
		 * {} public void mouseExited(MouseEvent e) {} public void
		 * mouseEntered(MouseEvent e) {}
		 * 
		 * public void mouseClicked(MouseEvent e) {
		 * 
		 * int row = invNoTable.getSelectedRow(); rcp = (InvViewDto)
		 * invNoTable.getValueAt(row, 2); fillData(rcp);
		 * 
		 * } });
		 */
		...
		/*
		 * JPanel panel_4 = new JPanel(); // panel_4.setBorder(new
		 * EtchedBorder(EtchedBorder.LOWERED, null, null));
		 * panel_4.setBorder(new LineBorder(new Color(0, 0, 0), 3));
		 * panel_4.setBackground(Color.LIGHT_GRAY); panel_4.setBounds(40, 134,
		 * 975, 48); getContentPane().add(panel_4);
		 */
		...
		// panel_1.setBorder(new EtchedBorder(EtchedBorder.LOWERED, null,
		// null));
		...
		btnSave.addKeyListener(new KeyAdapter() {
			public void keyPressed(KeyEvent evt) {
				if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
					...
					int ino = saveData();
					if (ino != 0) {
						JOptionPane.showMessageDialog(InwardEntryOld2.this,
								"Slip No " + billNo, "Slip No",
								JOptionPane.INFORMATION_MESSAGE);
					}
					vou_no.requestFocus();
		...
		try {
			for (int i = 0; i < size; i++) {
				...
				if (qty > 0) {
					...
					billNo = setIntNumber(vou_no.getText());
					...
					if (col.get(4) != null)
						rcp1.setSrate_net((setDoubleNumber(col.get(4)
								.toString())));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand().equalsIgnoreCase("print")) {
			...
			/*
			 * InvOpt copt = new
			 * InvOpt("com.coldstorage.print.GenerateInvoiceGST "
			 * ,"Inward Printing"); copt.setVisible(true);
			 */
			new GenerateInvoiceGST(String.valueOf(loginDt.getFin_year()),
					String.valueOf(loginDt.getDepo_code()),
					String.valueOf(loginDt.getDiv_code()), vou_no.getText(),
					vou_no.getText(), "PDF", loginDt.getDrvnm(), "", "2",
		...
		if (e.getActionCommand().equalsIgnoreCase("save")) {
			...
			if (ino != 0) {
				JOptionPane.showMessageDialog(InwardEntryOld2.this, "Slip No "
						+ billNo, "Slip No", JOptionPane.INFORMATION_MESSAGE);
			}
			vou_no.requestFocus();
			vou_no.setSelectionStart(0);
	...
	public int saveData() {
		...
		if ((option == 1 || option == 2)) {
			...
			if(vno>0)
			{
				...
				alertMessage(InwardEntryOld2.this, "Can't Save this Inward Vou No is 0 !!!!1");
		...
		if (option == 0) {
			...
			if (edit.equalsIgnoreCase("false"))
				alertMessage(InwardEntryOld2.this, "Can't Edit this Inward");
			else {
				int vno=setIntNumber(vou_no.getText());
	...
	public void itemTableUI() {
		...
		vouTable.setFont(new Font("c://windows//fonts//ARIALUNI.ttf",
				Font.BOLD, 14));
	...
	private void checkVou_no() {
		...
		if (vouno == 0) {
			...
			if (rcp != null) {
				...
				/*
				 * JOptionPane.showMessageDialog(InwardEntry.this,"Slip No. "+vouno
				 * ,"Already Exists!!!!!",JOptionPane.INFORMATION_MESSAGE);
				 * vou_no.setText(""); vou_no.requestFocus();
				 */
				option = 0;
				fillData(rcp);
				btnSave.setEnabled(true);
	...
	/*
	 * private void checkDateValidity(String date) { if(isValidDate(date)) {
	 * //lblinvalid.setVisible(false); // Check the Date Range for the Financial
	 * year if (isValidRange(date,loginDt.getFr_date(),loginDt.getTo_date())) {
	 * vac_code.requestFocus(); } else {
	 * JOptionPane.showMessageDialog(InwardEntry
	 * .this,"Date range should be in between: " +
	 * sdf.format(loginDt.getFr_date())+ " to " +
	 * sdf.format(loginDt.getTo_date()), "Date Range",
	 * JOptionPane.INFORMATION_MESSAGE);
	 * 
	 ...
	 * sdf.format(loginDt.getFr_date())+ " to " +
	 * sdf.format(loginDt.getTo_date())); vou_date.setValue(null);
	 * checkDate(vou_date); vou_date.requestFocus();
	 * 
	 * }
	 * 
	 * 
	 * } else { lblinvalid.setText("invalid date");
	 * lblinvalid.setForeground(Color.red); lblinvalid.setVisible(true);
	 * vou_date.setValue(null); checkDate(vou_date); vou_date.requestFocus(); }
	 * 
	 * }
	 */
	public int getLastNo() {
		int vno = 0;
		vno = idao.getInvoiceNo(loginDt.getDepo_code(),loginDt.getDiv_code(), loginDt.getFin_year(),
				60);
		return vno;
	}
