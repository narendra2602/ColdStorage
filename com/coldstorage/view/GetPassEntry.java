...
public class GetPassEntry extends BaseClass implements ActionListener {
	...
	private JLabel lblGatePassNo;

	private JTextField gatepass_no;

	private JFormattedTextField gatepass_date;

	private JLabel lblgateDate;
...
//		String[] crDrColName = { "Marka No", "Qty", "Manzil", "Gala No"};
		String[] crDrColName = { "Slip No","Date","Category","Group","Item","Pack","Outward Qty", "Outward Weight","Marka No", "GatePass Qty", "GatePass Weight", "","","","","","","","",""};
		String cdDrData[][] = {{}};
...
//		cddrPane.setBounds(279, 480, 530, 140);
		cddrPane.setBounds(40, 260, 975, 205);
...
*/
		lblLastNo = new JLabel("Last GatePass No:");
		lblLastNo.setForeground(Color.RED);
		lblLastNo.setBounds(800, 233, 150, 20);
		getContentPane().add(lblLastNo);
		
		lastNo = new JLabel("");
		lastNo.setForeground(Color.RED);
		lastNo.setBounds(950, 233, 51, 20);
...
*/
		...
		lblDispatchEntry = new JLabel("Gate Pass Entry");
...
//		lblDispatchEntry.setBounds(331, 94, 382, 22);
		...
		KeyListener keyListener = new KeyAdapter() 
		{
			public void keyPressed(KeyEvent keyEvent) 
			{
				...
				if (keyEvent.getKeyCode() == KeyEvent.VK_ENTER) 
				{
					...
					{
					case 0:
						gatepass_date.requestFocus();
						gatepass_date.setSelectionStart(0);
		...
		lblvaccode.setBounds(282, 165, 126, 20);
		getContentPane().add(lblvaccode);

		vac_code = new JTextField();
		vac_code.setBounds(408, 165, 86, 23);
		...
		vac_code.addKeyListener(new KeyAdapter() 
		{
			public void keyPressed(KeyEvent evt) 
			{
				...
				if(evt.getKeyCode() == KeyEvent.VK_ENTER)
				{
					...
					{
						clearAll();
						JOptionPane.showMessageDialog(GetPassEntry.this,"No Record for Khata No. "+party,"Record not found",JOptionPane.INFORMATION_MESSAGE);						
		...
		lblNarration.setBounds(282, 199, 126, 20);
		getContentPane().add(lblNarration);
		
		vnart_1 = new JTextField();
		vnart_1.setBounds(408, 199, 382, 23);
		...
		hintTextLabel.setBounds(506, 165, 231, 22);
		getContentPane().add(hintTextLabel);

		
		name = new JTextField();
		name.setName("2");
		name.setBounds(504, 165, 286, 23);
			...
			public void keyPressed(KeyEvent evt) {
				...
				if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
					if(name.getText().length()==0)
						alertMessage(GetPassEntry.this, "Please select Party!");
					else
		...
		chl_no.addKeyListener(new KeyAdapter() 
		{
			...
			public void keyPressed(KeyEvent evt) 
			{
				...
				if(evt.getKeyCode() == KeyEvent.VK_ENTER)
				{
					...
					if (vouno==0)
					{
						JOptionPane.showMessageDialog(GetPassEntry.this,"Challan No. "+vouno,"Empty!!!!",JOptionPane.INFORMATION_MESSAGE);						
						chl_no.setText("");
						chl_no.requestFocus();
					}
					else
					{						
						rcp =  idao.getChallanDetail(chl_no.getText(),loginDt.getFin_year(),400,loginDt.getDepo_code(),loginDt.getDiv_code());
						...
						if(rcp!=null)
						{
							if(option==1)
							{
							JOptionPane.showMessageDialog(GetPassEntry.this,"Challan No. "+vouno,"Already Exists!!!!!",JOptionPane.INFORMATION_MESSAGE);						
							chl_no.setText("");
							chl_no.requestFocus();
							}
							else
						...
						{
							JOptionPane.showMessageDialog(GetPassEntry.this,"Challan No."+billNo," Not Found!!!!!",JOptionPane.INFORMATION_MESSAGE);						
							chl_no.setText("");
							chl_no.requestFocus();
						}
						evt.consume();
		...
		lblchlDate.setBounds(540, 131, 70, 20);
		...
		chl_date.setBounds(604, 131, 185, 22);
		getContentPane().add(chl_date);

		
		
		lblGatePassNo = new JLabel("Gate Pass No:");
		lblGatePassNo.setBounds(282, 233, 130, 25);
		getContentPane().add(lblGatePassNo);

		
		gatepass_no = new JTextField();
		gatepass_no.setHorizontalAlignment(SwingConstants.RIGHT);
		gatepass_no.setBounds(408, 233, 125, 23);
		getContentPane().add(gatepass_no);
		gatepass_no.addKeyListener(new KeyAdapter() 
		{
			...
			public void keyPressed(KeyEvent evt) 
			{
				...
				if(evt.getKeyCode() == KeyEvent.VK_ENTER)
				{
					int vouno = setIntNumber(gatepass_no.getText().toString());
					 
					if (vouno==0)
					{
						JOptionPane.showMessageDialog(GetPassEntry.this,"Gate Pass No. "+vouno,"Empty!!!!",JOptionPane.INFORMATION_MESSAGE);						
						gatepass_no.setText("");
						gatepass_no.requestFocus();
					}
					else
					{						
						rcp =  idao.getChallanDetail(gatepass_no.getText(),loginDt.getFin_year(),50,loginDt.getDepo_code(),loginDt.getDiv_code());
						...
						if(rcp!=null)
						{
							if(option==1)
							{
							JOptionPane.showMessageDialog(GetPassEntry.this,"Gate Pass No. "+vouno,"Already Exists!!!!!",JOptionPane.INFORMATION_MESSAGE);						
							chl_no.setText("");
							chl_no.requestFocus();
							}
							else
						...
						{
							JOptionPane.showMessageDialog(GetPassEntry.this,"Gate Pass No."+billNo," Not Found!!!!!",JOptionPane.INFORMATION_MESSAGE);						
							gatepass_no.setText("");
							gatepass_no.requestFocus();
						}
						evt.consume();
		...
		lblgateDate = new JLabel("Date:");
		lblgateDate.setBounds(540, 233, 70, 20);
		getContentPane().add(lblgateDate);


		gatepass_date = new JFormattedTextField(sdf);
		gatepass_date.setFont(new Font("Tahoma", Font.BOLD, 11));
		checkDate(gatepass_date);
		gatepass_date.setText(sdf.format(new Date()));
		gatepass_date.setBounds(604, 233, 185, 22);
		getContentPane().add(gatepass_date);

		gatepass_date.addFocusListener(new FocusListener() {
			
			@Override
			public void focusLost(FocusEvent arg0) {
				String date=gatepass_date.getText();
				checkDateValidity(date, gatepass_date, CrDrTable, GetPassEntry.this);
...
//		weightout.setName("3");
		...
		gatepass_date.addKeyListener(keyListener);
...
//		weightout.addKeyListener(keyListener);
		...
		gatepass_date.addFocusListener(myFocusListener);
		gatepass_date.addFocusListener(myFocusListener);
				...
				if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
					if (column == 9)
					{
						...
						else
						{
							...
							{
								alertMessage(GetPassEntry.this, "Quantity should not be greater than "+sqty);
					...
					if (column == 10)
					{
						...
						else
						{
							...
							if(row+1==totRow)
							{
								...
								{
									alertMessage(GetPassEntry.this, "Weight should not be greater than "+sweight);
...
*/						}
		...
		btnAdd = new JButton("Edit by Gate Pass No");
		btnAdd.setActionCommand("Edit");
		btnAdd.setBounds(42, 605, 190, 30);
		getContentPane().add(btnAdd);

		btnDelete = new JButton("Delete by Gate Pass No");
		btnDelete.setActionCommand("Delete");
		btnDelete.setBounds(240, 605, 200, 30);
		...
		btnSave.addKeyListener(new KeyAdapter() {
			public void keyPressed(KeyEvent evt) {
				if (evt.getKeyCode() == KeyEvent.VK_ENTER) 
				{
					...
					if(ino!=0)
					{
						JOptionPane.showMessageDialog(GetPassEntry.this,"Gate Pass No "+billNo,"Gate Pass No",JOptionPane.INFORMATION_MESSAGE);
					}
					chl_no.requestFocus();
					chl_no.setSelectionStart(0);
					lastNo.setText(String.valueOf(getLastNo()));
...
//		checkDate(chl_date);
		...
		try {
			for (int i = 0; i < size; i++) {
				...
				if(qty>0)
				{
					...
					billNo=setIntNumber(gatepass_no.getText());
					rcp1.setDepo_code(loginDt.getDepo_code());
					rcp1.setDoc_type(50);
					
					if(option==0 || option==3  )  // edit
					{
					...
					{
						rcp1.setSinv_no(setIntNumber(chl_no.getText()));
						rcp1.setInv_date(sdf.parse(chl_date.getText()));
					}
					...
					rcp1.setChl_no(gatepass_no.getText());
					rcp1.setChl_date(sdf.parse(gatepass_date.getText()));
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		if(e.getActionCommand().equalsIgnoreCase("edit"))
		{
			option=0;
			gatepass_no.requestFocus(); 
			gatepass_no.setSelectionStart(0);
	...
	public int saveData() 
	{
		...
		if (option==3)
		{
	    		...
	    		int answer = JOptionPane.showOptionDialog(GetPassEntry.this, "Are You Sure : ","Confirm Yes/No",JOptionPane.YES_NO_OPTION,
...
//		vou_date.setText(sdf.format(rcp.getInv_date()));
		...
		try {
			for (int i = 0; i < size; i++) {
				...
				if (setIntNumber(col.get(9).toString().trim())!=0 ) {
					...
					trd.setChl_no(gatepass_no.getText());
					trd.setChl_date(sdf.parse(gatepass_date.getText()));
	...
	{
		int vno=0;
		vno=idao.getInvoiceNo(loginDt.getDepo_code(),loginDt.getDiv_code(),loginDt.getFin_year(), 50);
		return vno;
	}
