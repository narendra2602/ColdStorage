...
public class AddFavourite extends BaseClass  implements ActionListener 
{
...
*/
        ...
        userDataList =userDao.getUserForFavourite(loginDt.getLogin_id());
		...
		saveButton.setActionCommand("saveButton");
 		...
 		lblLimit = new JLabel("Please select maximum 10 Records Only. ");
 		lblLimit.setBounds(20, 530, 500, 20);
 		lblLimit.setForeground(Color.RED);
 		lblLimit.setFont(new Font("Tahoma", Font.BOLD, 18));
		...
		userSearch = new JTextField(loginDt.getLogin_id());
		userSearch.setEditable(false);
		...
		lblLoginInformation.setBounds(670, 16, 150, 20);
		...
		lblPersonalInformation.setBounds(23, 14, 153, 20);
...
*/
	...
	public void setDataOnScreen(LoginDto ldto)
	{
		...
		userSearch.setText(""+loginDt.getLogin_id());
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		if(command.equalsIgnoreCase("saveButton"))
		{
			...
			if(loginDto!=null)
			{
				...
				int userId=0;
				if(loginDto.getReportList()!=null)
				  userId=userDao.updateFavourite(loginDto);
				if(userId!=0)
				{
...
*/	
	...
	public void fillEntryAndReportTable(List<UserDto> reportList)
	{
		...
		for(int i =0;i<size;i++)
		{
			...
			{
				EntryTableModel.addRow(new Object[]{userDto.getFav_type().equals("F") ? true : false,userDto.getPack_name(),userDto.getTab_name(),userDto.getMenu_name(),userDto});
			}
			if(userDto.getTran_type().equals("R"))
			{
				ReportTableModel.addRow(new Object[]{userDto.getFav_type().equals("F") ? true : false,userDto.getPack_name(),userDto.getTab_name(),userDto.getMenu_name(),userDto});
			}
		}
	}
...
//				udt.setStatus("N");
		...
		if(count>9)
			JOptionPane.showMessageDialog(this, "Please select upto 10 only. ");
		else
		{
			response[1]=dataList;
