...
/*			String subQ="select menu_code,menu_name,class_name,sub_class,ifnull(fav_type,'') from menumaster where pack_code=? and menu_code in" +
					"(select distinct(menu_code) from userrights where user_id=? and pack_code=? and tab_code=?  and status='Y') order by serialno";
*/	
			
			String subQ=" select m.menu_code,m.menu_name,m.class_name,m.sub_class,ifnull(u.fav_type,'')" +
					" from menumaster m,userrights u "+  
			" where m.pack_code=?  and m.pack_code=u.pack_code and m.menu_code=u.menu_code" +
			" and m.tab_code=u.tab_code and u.user_id=?  and m.tab_code=? and u.status='Y' order by m.serialno";
			...
			while (rs.next())
			{
				...
				while(rs1.next())
				{
					...
					sm.setFav_type(rs1.getString(5));
					if(rs1.getString(5).equalsIgnoreCase("F"))
						wcount++;
					s = new ArrayList();
					while(rs2.next())
					{
...
//						s.put(rs.getString(1)+rs1.getString(1)+rs2.getString(1),rs2.getString(2));
		       ...
		       MenuDto mdto1 = (MenuDto) l.get(0);
		       mdto1.setFavCount(((wcount+1)*35)+4);
		       l.set(0, mdto1);
