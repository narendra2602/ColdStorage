...
public class TransportDAOold1 
{
	
	public List getTransport(int depo_code,String nm)
	{
		...
		try {
			...
			String query2 ="select account_no,mac_name,mac_name_hindi,madd1,madd2,madd3,mcity,mcity_hindi,mpin,mstate,mphone,mobile,memail,mcontact,aadhar_no," +
					"pan_no from accountmaster where  mac_name  like ? order by account_no";  
	...
	public HashMap getPartyNameMap(int depo)
	{
		PreparedStatement ps2 = null;
		ResultSet rs=null;
		Connection con=null;
      ...
      public int updateTransport(TransportDto tdto)
      {
    	...
    	PreparedStatement ps2 = null;
  		...
  		try 
  		{
  			...
  			"mcontact=?,mstate=?,aadhar_no=?,pan_no=?,mac_name_hindi=?,mcity_hindi=? where  account_no=? ";
    ...
    public Vector getProductList(int group_code)
    {
  	...
  	PreparedStatement ps2 = null;
		...
		try 
		{
			...
			" p.rate "+
			" from prd as p , grpmast as gg where p.pd_group=? and "+ 
			" p.pd_group=gg.gp_code  order by p.pname";
			
			ps2 = con.prepareStatement(query2);

			ps2.setInt(1, group_code);
			rs= ps2.executeQuery();
			con.setAutoCommit(false);
			v = new Vector ();
			while (rs.next())
    ...
    public Vector getProductListAll(int group_code)
    {
  	...
  	PreparedStatement ps2 = null;
		...
		try 
		{
			...
			query2 ="select p.pcode,p.pname,p.pack,ifnull(p.pname_hindi,''),p.pd_group,gg.gp_name,"+
			" p.rate "+
			" from prd as p , grpmast as gg where  p.pd_group=gg.gp_code  order by p.pname";
    ...
    public Vector getTransportList(int depo)
	{
		PreparedStatement ps2 = null;
		ResultSet rs=null;
		Connection con=null;
	...
	public int  checkParty(int depo_code,String  nm,String city)
	{
		PreparedStatement ps2 = null;
		ResultSet rs=null;
		Connection con=null;
		...
		try {
			...
			String query2 = "select account_no from accountmaster where  depo_code=? and mac_name=? and mcity=?" ;
